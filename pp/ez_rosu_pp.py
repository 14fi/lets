...
class EzRosu:
    ...
    def _calculatePP(self):
        ...
        try:
            ...
            map = Beatmap(path = mapFile)
            rosu =  Calculator(mods = self.mods, mode=self.gameMode)
            max_perf = rosu.performance(map)
            rosu.set_difficulty(max_perf.difficulty)

            # Not so readeable part starts here...
            if self.misses > 0:
                rosu.set_n_misses(self.misses)
            if self.combo >= 0:
                rosu.set_combo(self.combo)
            if not self.tillerino:
                if self.acc > 0:
                    rosu.set_acc(self.acc)

            if self.mods > mods.NOMOD:
                rosu.set_mods(self.mods)
            
            rosu_calculations = rosu.performance(map)
            map_calculations = rosu.difficulty(map)
            if not self.tillerino:
                if rosu_calculations.mode != self.score.gameMode:
                    self.pp = 0
                else:
                    temp_pp = round(rosu_calculations.pp, 5)
                    self.stars = map_calculations.stars
