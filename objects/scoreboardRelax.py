...
class scoreboardRelax:
	...
	def setScores(self):
		...
		if beatmap.rankedStatus == 5 or self.mods & modsEnum.AUTOPLAY == 0:
	...
	def setPersonalBestRank(self):
		...
		query = "SELECT id FROM scores_relax WHERE beatmap_md5 = %(md5)s AND userid = %(userid)s AND play_mode = %(mode)s AND completed = 3"
		# Mods
		if self.mods > -1:
